...
print(f'//*[@for="{config.type_residence_permit[:-8]}"]')
...
def catch_termin_page(browser):
    try:
        wait_find(
            browser, f'//*[@for="{config.type_residence_permit[:-8]}"]', attribute=By.XPATH, timeout=5)
    except TimeoutError:
        for i in range(3):
...
with webdriver.Chrome(service=s, options=options) as driver:
    ...
    time.sleep(delay_medium)
    Select(select_citizenship).select_by_value(config.code_citizenship)

    time.sleep(delay_short)
    select_number = driver.find_element(By.ID, "xi-sel-422")
    time.sleep(delay_short)
    Select(select_number).select_by_value(config.count_applicants)
    ...
    time.sleep(delay_short)
    Select(select_partner).select_by_value(config.code_citizenship_partner)

    time.sleep(delay_short)
    radio_extend = driver.find_element(
        By.XPATH, f'//*[@for="{config.type_residence_permit[:-10]}"]')
    # time.sleep(delay_short)
    radio_extend.click()

    time.sleep(delay_short)
    radio_purpose_studies = driver.find_element(
        By.XPATH, f'//*[@for="{config.type_residence_permit[:-8]}"]')
    # time.sleep(delay_short)
    radio_purpose_studies.click()

    time.sleep(delay_short)
    radio_purpose_study = driver.find_element(
        By.ID, config.type_residence_permit)
    # time.sleep(delay_short)
